(function () {
  ...
  function addLiveStyles() {
    ...
    style.textContent = [
      ...
      ".image-preview-overlay{display:none;position:fixed;inset:0;z-index:20000;background:rgba(2,8,14,.92);padding:calc(14px + env(safe-area-inset-top)) 14px calc(14px + env(safe-area-inset-bottom));overflow:auto}",
      ".image-preview-overlay.visible{display:flex;align-items:flex-start;justify-content:center}",
      ".image-preview-box{width:min(430px,100%);margin:auto;background:#08131f;border:1px solid #244b6d;border-radius:16px;padding:10px;box-shadow:0 18px 55px rgba(0,0,0,.55)}",
      ".image-preview-box img{display:block;width:100%;height:auto;border-radius:10px;background:#06111b}",
      ".image-preview-meta{text-align:center;color:#7f96ad;font-size:9px;font-weight:800;margin:8px 0}",
      ".image-preview-actions{display:grid;grid-template-columns:1fr 1fr;gap:7px}",
      ".image-preview-actions button{min-height:42px;border-radius:9px;font-weight:950;border:1px solid #2b5277;background:#0d2135;color:#eaf7ff}",
      ".image-preview-actions .share-image{background:linear-gradient(180deg,#2ba9ff,#0874df);border:0}",
      ".image-preview-close{width:100%;margin-top:7px;min-height:40px;border-radius:9px;border:1px solid #233f59;background:#0b1724;color:#9fb4ca;font-weight:900}"
    ].join("");
  ...
  function fitCanvasText(ctx, text, maxWidth, startSize, minSize, weight) {
    ...
    while (size > minSize) {
      ctx.font = (weight || '900') + ' ' + size + 'px ' + family;
      if (ctx.measureText(text).width <= maxWidth) return size;
      size -= 1;
    }
    ctx.font = (weight || '900') + ' ' + minSize + 'px ' + family;
    return minSize;
  }

  function getBannerImageSource() {
    const banner = document.querySelector(".banner");
    if (!banner) return "";
    const bg = banner.style.backgroundImage || getComputedStyle(banner).backgroundImage || "";
    const match = bg.match(/^url\((['"]?)(.*)\1\)$/);
    return match ? match[2] : "";
  }

  function loadPosterBanner() {
    return new Promise(function(resolve) {
      const src = getBannerImageSource();
      if (!src) {
        resolve(null);
        return;
      }
  ...
  function groupLaunchersForImage() {
    const groups = [];
    const byName = {};

    currentResultApcs.forEach(function(item) {
      let g = byName[item.player];
      if (!g) {
        g = { name: item.player, apcs: [] };
        byName[item.player] = g;
        groups.push(g);
      }

      const label = apcLabel(item.vehicle);
      if (g.apcs.indexOf(label) === -1) g.apcs.push(label);
    });

    return groups;
  }

  function drawPosterRow(ctx, x, y, w, h, item) {
    roundedRectPath(ctx, x, y, w, h, 12);
    ctx.fillStyle = "rgba(8,29,45,.94)";
    ctx.fill();
    ctx.lineWidth = 1.5;
    ctx.strokeStyle = "rgba(67,188,247,.72)";
    ctx.stroke();

    const nameX = x + 16;
    const apcText = item.apcs.join(" / ");
    const rightX = x + w - 16;
    const dividerX = x + Math.round(w * .58);

    ctx.fillStyle = "#f3f8ff";
    fitCanvasText(ctx, item.name, Math.max(90, dividerX - nameX - 16), 22, 15, "900");
    ctx.textAlign = "left";
    ctx.textBaseline = "middle";
    ctx.fillText(item.name, nameX, y + h / 2);

    ctx.strokeStyle = "rgba(62,156,207,.55)";
    ctx.lineWidth = 1;
    ctx.beginPath();
    ...
    fitCanvasText(ctx, apcText, Math.max(80, rightX - dividerX - 12), 20, 14, "900");
    ctx.textAlign = "right";
    ctx.fillText(apcText, rightX, y + h / 2);
  }

  function createPreviewOverlay() {
    let overlay = document.getElementById("frankyImagePreview");
    if (overlay) return overlay;

    overlay = document.createElement("div");
    overlay.id = "frankyImagePreview";
    overlay.className = "image-preview-overlay";
    overlay.innerHTML =
      '<div class="image-preview-box">' +
        '<img id="frankyGeneratedImage" alt="ABYX rally launchers image">' +
        '<div class="image-preview-meta" id="frankyImageMeta"></div>' +
        '<div class="image-preview-actions">' +
          '<button type="button" class="share-image" id="shareGeneratedImage">SHARE</button>' +
          '<button type="button" id="saveGeneratedImage">SAVE</button>' +
        '</div>' +
        '<button type="button" class="image-preview-close" id="closeGeneratedImage">CLOSE</button>' +
      '</div>';

    document.body.appendChild(overlay);

    document.getElementById("closeGeneratedImage").onclick = function() {
    ...
    document.getElementById("saveGeneratedImage").onclick = function() {
      if (!generatedImageBlob) return;
      const url = generatedImageUrl || URL.createObjectURL(generatedImageBlob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "ABYX_FRANKY_" + currentResultApcs.length + "_RALLIES.jpg";
      document.body.appendChild(a);
      a.click();
      a.remove();
    };

    document.getElementById("shareGeneratedImage").onclick = function() {
      if (!generatedImageBlob) return;

      const file = new File(
        [generatedImageBlob],
        "ABYX_FRANKY_" + currentResultApcs.length + "_RALLIES.jpg",
        { type: "image/jpeg" }
      );
      ...
      if (navigator.share && (!navigator.canShare || navigator.canShare({ files: [file] }))) {
        navigator.share({
          ...
          text: currentResultApcs.length + " rallies to be sent!",
          files: [file]
        }).catch(function(){});
      } else {
        document.getElementById("saveGeneratedImage").click();
      }
    };
  ...
  function generateRallyImage() {
    if (!currentResultApcs.length) {
      alert(txt(
        "Génère d’abord un résultat avec au moins un rally.",
        "Build a rally plan first.",
        "Crea prima un piano rally.",
        "Erstelle zuerst einen Rally-Plan."
      ));
      return;
    }
    ...
    loadPosterBanner().then(function(bannerImg) {
      ...
      bg.addColorStop(0, "#071826");
      bg.addColorStop(.55, "#07121d");
      ...
      // Poster art area.
      if (bannerImg) {
        drawCover(ctx, bannerImg, 0, 0, W, 390);
      } else {
        const art = ctx.createRadialGradient(600, 90, 20, 600, 120, 520);
        art.addColorStop(0, "#17669a");
        art.addColorStop(.55, "#0b2a42");
      ...
      shade.addColorStop(.55, "rgba(2,10,17,.28)");
      shade.addColorStop(1, "rgba(2,10,17,.98)");
      ctx.fillStyle = shade;
      ctx.fillRect(0, 0, W, 410);

      // ABYX mark.
      ctx.save();
      ctx.translate(42, 42);
      ctx.strokeStyle = "rgba(44,202,255,.55)";
      ctx.lineWidth = 5;
      ctx.beginPath();
      ctx.moveTo(0, 5); ctx.lineTo(70, 5); ctx.lineTo(96, 35); ctx.lineTo(70, 65); ctx.lineTo(0, 65);
      ctx.stroke();
      ctx.restore();
      ...
      const panelW = W - 76;
      const panelH = 590;
      roundedRectPath(ctx, panelX, panelY, panelW, panelH, 22);
      ctx.fillStyle = "rgba(4,18,29,.96)";
      ctx.fill();
      ctx.lineWidth = 2;
      ctx.strokeStyle = "rgba(50,187,245,.72)";
      ctx.stroke();

      const groups = groupLaunchersForImage();
      ...
      const listTop = panelY + 48;
      const listH = 500;
      const gapY = rowsPerCol > 10 ? 3 : 6;
      const rowH = Math.max(29, Math.min(54, Math.floor((listH - gapY * Math.max(0, rowsPerCol - 1)) / Math.max(1, rowsPerCol))));

      // Small list header.
      ctx.textAlign = "left";
      ctx.fillStyle = "#8eb5cf";
      ctx.font = "800 13px Arial, Helvetica, sans-serif";
      ctx.fillText("LAUNCHER", innerX + 14, panelY + 31);
      if (colCount === 1) {
        ctx.textAlign = "right";
        ctx.fillText("APC", innerX + colW - 14, panelY + 31);
      } else {
        ctx.textAlign = "right";
        ctx.fillText("APC", innerX + colW - 14, panelY + 31);
        ctx.fillText("APC", innerX + colW + gapX + colW - 14, panelY + 31);
      }
      ...
      groups.forEach(function(item, index) {
        ...
        drawPosterRow(ctx, x, y, colW, rowH, item);
      });

      // Join line.
      const joinY = 1016;
      roundedRectPath(ctx, 95, joinY, W - 190, 56, 18);
      ctx.fillStyle = "rgba(10,39,59,.96)";
      ctx.fill();
      ctx.strokeStyle = "rgba(64,203,255,.52)";
      ctx.lineWidth = 1.5;
      ctx.stroke();
      ...
      ctx.fillText("All the other plz join !", W / 2, joinY + 28);

      // Bottom rally count banner.
      const callY = 1092;
      ctx.fillStyle = "#48ddff";
      ctx.fillRect(40, callY, W - 80, 76);

      ctx.fillStyle = "#021018";
      fitCanvasText(
        ctx,
        currentResultApcs.length + " RALLIES TO BE SENT !!",
        W - 130,
        42,
        29,
        "900"
      ...
      ctx.textAlign = "center";
      ctx.textBaseline = "middle";
      ctx.fillText(currentResultApcs.length + " RALLIES TO BE SENT !!", W / 2, callY + 38);

      ctx.fillStyle = "#678aa3";
      ctx.font = "800 12px Arial, Helvetica, sans-serif";
      ctx.fillText("ABYX  ·  FRANKY EVENT", W / 2, 1190);
      ...
      canvas.toBlob(function(blob) {
        ...
        document.getElementById("frankyGeneratedImage").src = generatedImageUrl;
        document.getElementById("frankyImageMeta").textContent =
          currentResultApcs.length + " rallies · " +
          groups.length + " launchers · " +
  ...
  function setupImageGenerator() {
    ...
    button.addEventListener("click", generateRallyImage);
  }
